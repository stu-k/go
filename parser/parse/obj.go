...
func (o *Obj) Parse(s string) (Data, string, error) {
	if err := errors.CheckInit(o.Type(), s, o.Check); err != nil {
	...
	for i := 0; i < len(toparse); i++ {
		...
		case r == '}':
			...
			if key != "" || isComma {
				return errors.HandleError(errors.NewUnexpectedCharErr('}'))
			}
		...
		default:
			...
			case "var":
				...
				continue
			default:
				return errors.HandleError(fmt.Errorf("invalid obj key type: %s", data.Type()))
			}
		}
	}
